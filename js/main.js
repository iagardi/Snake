...
const head = {
  ...
  updateSeconds() {
    const pad = second => {
      // to pad single digit values with a 0 in the front
      let check = second + "";
      if (check.length < 2) {
        return "0" + second;
      } else {
  ...
  positionSnake() {
    // position the snake at each interval based on the current direction
    let row = this.row;
  ...
  addTail() {
    const bobAteAFruit = new Growsnake(head.prevRow, head.prevCol, head.length, 0, 0);
